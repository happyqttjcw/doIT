...
public class PageService {
	...
	public List<Setting> getsameuser(int comno) {
		...
		for (int i = 0; i < allUserSet.size(); i++) {
			...
			if (allUserSet.get(i).getSkill() != null)
				userskills = allUserSet.get(i).getSkill().split(",");
			String[] userwelfares = null;
			if (allUserSet.get(i).getWelfare() != null)
				userwelfares = allUserSet.get(i).getWelfare().split(",");
			String[] userpluses = null;
			if (allUserSet.get(i).getPluse() != null)
				userpluses = allUserSet.get(i).getPluse().split(",");
			String[] userlocs = null;
			if (allUserSet.get(i).getLocation() != null)
				userlocs = allUserSet.get(i).getLocation().split(",");
			String[] userjobs = null;
			if (allUserSet.get(i).getJob() != null)
				userjobs = allUserSet.get(i).getJob().split(",");
